class HomeController < ApplicationController
  ...
  def roll
    ...
    if @summoner.vortex_key <= 0
      render text: "No keys to open this vortex!"
    else
      ...
      case
        ...
        #   @summoner.gp += @gp
        #   if @summoner.save
        #     render text: "You won #{@gp}"
        #   else
        #   end
        #   #update a database with results

        when (1..1000).include?(reward_category_roll) #10% ability
          ...
          @ability_purchases = AbilityPurchase.includes(:ability)
          @summoner.save
